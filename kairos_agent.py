...
def execute_trade_on_polygon(details):
    ...
    logging.info(f"Polygon trade sent! Tx hash: {tx_hash.hex()}")
    return tx_hash.hex()
...
def execute_trade_on_hedera(hbar_amount, to_account_id):
    ...
    if not hedera_account_id or not hedera_private_key:
        logging.error("Hedera credentials missing.")
        return None
    ...
    try:
        ...
        logging.info(f"Executing real Hedera trade: Transfer {hbar_amount} HBAR from {hedera_account_id} to {to_account_id}")
        tx_response = transaction.execute(client)
        receipt = tx_response.getReceipt(client)
        
        logging.info(f"Hedera trade executed! Status: {receipt.status}, Transaction ID: {receipt.transactionId}")
        return str(receipt.transactionId)
        
    except Exception as e:
        logging.error(f"Error executing Hedera trade: {e}")
        return None
...
if __name__ == "__main__":
    ...
    while True:
        ...
        hedera_trade_amount = 10
        hedera_to_account = "0.0.2"
        
        base_profit = hedera_trade_amount * 0.02
        market_variance = random.uniform(-0.01, 0.03) * hedera_trade_amount
        hedera_profit = base_profit + market_variance
